...
import { fetchDriverStandings, type DriverStanding } from '../../services/jolpica';
      ...
      {standings.map((s) => {
        ...
        return (
          <div
            ...
            <div className={`w-1 h-8 rounded-full ${getTeamColor(teamId)}`} />

            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-gray-900 dark:text-gray-100">
                {s.Driver.givenName} <span className="font-semibold">{s.Driver.familyName}</span>
                {s.Driver.code && (
                  <span className="ml-2 text-xs text-gray-400 font-mono">{s.Driver.code}</span>
                )}
              </p>
            ...
            {Number(s.wins) > 0 && (
              <div className="text-right shrink-0 ml-2">
                <p className="text-sm font-medium text-gray-700 dark:text-gray-300">{s.wins}</p>
